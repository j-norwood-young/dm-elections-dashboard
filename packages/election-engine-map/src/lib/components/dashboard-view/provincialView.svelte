  ...
  function getTotalParty(data) {
    ...
    const result = provinces_geo_data.map((feature) => {
      ...
      if (matchingPartyResult) {
        ...
        feature.properties.total_valid_votes = matchingPartyResult.total_valid_votes;
        feature.properties.registered_voters = matchingPartyResult.registered_votes;
        feature.properties.total_votes_cast = matchingPartyResult.total_votes_cast;
      }
      return feature;
    });
    return result;
  ...
  {#if provinces_array}
    ...
    <div bind:clientWidth={svgWidth} bind:clientHeight={svgHeight}>
        ...
        <g>
          {#each provinces_array as municipality, index}
            <g>
              ...
              <path
                on:mouseover={(e) => {
                  if (!municipality.properties.highest_parties?.length) return;
                  ...
                  (provincialPopOverData["color"] = municipality.properties.highest_parties[0].party_color), index;
                }}
        ...
        <g transform="translate({INSET_POS[selected_region]})">
          <svg class="electionengine-national-map-inset" width="120" height="100" viewBox="0 0 500 600">
            <g class="national_map">
              <path d={national_path(national_map)} fill="none" stroke="#444444" stroke-width="0.8" />
            </g>
      ...
      {#if provincialPopOverData}
        <ProvincialPopOver bind:provincialPopOverData bind:svgHeight bind:svgWidth />
      {/if}
    </div>
  {/if}
</div>
