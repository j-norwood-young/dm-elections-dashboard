<script>
    ...
    $: init = () => {
        cartogram_data = provinces.map((d) => {
            ...
            const region_seat = regionalSeatAllocation.filter(
                (e) => d.properties.PROVINCE === e.region
            )[0];
...
<div class="electionengine-cartogram" class:electionengine-mb-grid={grid}>
    {#each cartogram_data as hexagon_data}
        <div
            class="electionengine-block"
            class:electionengine-grid-aligncenter={grid}
            style="left:{hexagon_data.x -
                hexagon_data.width / 2}px; top:{hexagon_data.y -
                hexagon_data.height / 2}px;"
        >
            <Hexagons
